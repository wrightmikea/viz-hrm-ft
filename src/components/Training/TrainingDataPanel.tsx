  ...
  const renderMiniEpisode = (episode: Episode, index: number) => {
    ...
    return (
      <motion.div
        ...
        className={`p-1 border rounded text-center bg-gray-50 ${isHighlighted ? 'border-blue-500 shadow' : 'border-gray-300'}`}
      ...
      >
        ...
        <div className="text-xs text-gray-600">{episode.actions.length}s</div>
        <div className={`text-xs ${episode.success ? 'text-green-600' : 'text-red-600'}`}>
          {episode.success ? '✓' : '✗'}
        </div>
      </motion.div>
    );
